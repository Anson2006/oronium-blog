const API_BASE = "https://6a50f8fbc576c846dcba1291.mockapi.io";
...
async function seed() {
  console.log("Seeding posts to mockapi.io...");
  for (const post of mockPosts) {
    try {
      // We will POST the post. mockapi.io automatically assigns IDs, but we can try to send id as well.
      // Note: mockapi.io might auto-increment or use string IDs. We will send all properties.
      const res = await globalThis.fetch(`${API_BASE}/posts`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(`Failed to POST post ${post.id}: ${res.statusText}`);
      }
      const data = await res.json();
      console.log(`Successfully seeded post ${post.id} as mockapi ID ${data.id}`);
    } catch (err) {
      console.error(err);
    }
  }
